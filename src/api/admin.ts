...
export function getAdmins(form: { page_index: number; page_size: number; sort: boolean; order: string }) {
    return request.get(`/backend/v1/admins`, {
        params: {
...
export function createAdmin(form: { username: string; password: string }) {
    return request.post(`/backend/v1/admins`, {
...
export function editAdmin(form: { id: number; username: string }) {
    return request.put(`/backend/v1/admins/${form.id}`, {username: form.username})
}

export function deleteAdmin(form: { id: number }) {
    return request.delete(`/backend/v1/admins/${form.id}`)
}

export function resetAdminPassword(form: { id: number }) {
    return request.get(`/backend/v1/admins/password/reset/${form.id}`)
}

export function changePassword(password: string) {
    return request.put(`/backend/v1/admins/password/change`, {
        password: password,
    })
}

export function changeRole(form: { id: number; role_id: number }) {
    return request.put(`/backend/v1/admins/roles/change/${form.id}`, {
        role_id: form.role_id,
    })
}

export function changeAdminAvatar(form: { name: string }) {
    return request.put(`/backend/v1/admins/avatar`, {
        avatar: form.name
    })
}
